...
Aqui está uma seção em **inglês** pronta para ser colada diretamente no seu arquivo `AGENTS.md`. Ela orienta os agentes (como Claude Code, OpenCode, Codex, Aider, Cursor, Kimi, etc.) a utilizarem o **Graphify** como fonte primária para entender a arquitetura, dependências e navegação no projeto antes de fazer buscas genéricas (`grep`/`find`).

## Codebase Knowledge Graph (Graphify Instructions)

This repository uses **Graphify** to index code, documentation, and architecture into a deterministic Knowledge Graph stored in `graphify-out/graph.json`.

### Core Directive

**ALWAYS query the knowledge graph before attempting raw file searches or directory grepping** when answering architectural, dependency, or structural questions about this codebase.

### Available Graphify Tools & Commands

Use the following Skill commands to inspect the codebase graph:

1. **Query Scoped Subgraphs**
   `/graphify query "<your question or concept>"`

_Use case:_ Ask natural language questions about how modules interact or how features are structured (e.g., `/graphify query "how are historical figures handled?"`).

2. **Trace Paths Between Symbols**

`/graphify path "<SourceSymbol>" "<TargetSymbol>"`

_Use case:_ Trace dependencies, call chains, or data flow between two concepts/files (e.g., `/graphify path "UserService" "DatabasePool"`). 3. **Explain Specific Concepts / Symbols**

`/graphify explain "<SymbolName>"`

_Use case:_ Inspect a single symbol, its exact source lines, community grouping, degree of connectivity, and explicit (`EXTRACTED`) vs. inferred (`INFERRED`) relationships. 4. **High-Level Architectural Overview**

- Review `graphify-out/GRAPH_REPORT.md` for God Nodes (most connected central concepts), subsystem communities, and key architectural highlights.

### Edge Confidence Guidelines

When reviewing graph outputs:

- `EXTRACTED`: Explicit syntax relationships parsed directly from AST (e.g., `imports`, `calls`, `inherits`).
- `INFERRED`: Derived semantic connections. Always verify critical inferred logic against the target source file if precision is mandatory.

### Agent Workflow Rules

- **First Step for Architecture/Refactoring:** Run `/graphify query` or `/graphify explain` to map affected symbols and packages.
- **Targeted File Reads:** Only read full raw source files after identifying the precise files and locations from the graph query results.
- **Avoid Full Workspace Scans:** Do not invoke heavy `grep`, `find`, or blanket directory scans if the relationship can be retrieved from `graphify-out/graph.json`.
