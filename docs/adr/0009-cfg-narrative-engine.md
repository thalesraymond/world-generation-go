...
3. **Engine** (engine.go) — `Resolve(ruleName, context, rng)` expands a rule by first filtering alternatives to those **eligible** (every direct `$variable` present-and-non-empty in context), then selecting uniformly among the eligible set with `rng.IntN(len(eligible))`. If no alternative is eligible, the sentinel `ErrNoEligibleAlternative` (wrapped with the rule name, comparable via `errors.Is`) is returned — no backtracking, callers implement fallback chains. Non-terminals are resolved recursively up to a recursion cap (`defaultMaxDepth = 10`); over-deep recursion falls back to `[...]`. Variables absent from context after eligibility filtering emit `$name` literally (defense in depth; unreachable under valid grammar/context).
...
- Variable substitution is explicit (context map); eligibility filtering prevents alternatives with missing variables from being selected, so `$name` literal emission is a defense-in-depth fallback unreachable under valid grammar/context.
