...
func TestPostProcessAssignsEventIDs(t *testing.T) {
	...
	if err := PostProcess(nil, events, SignificanceContext{}, TransferContext{}, nil); err != nil {
...
func TestPostProcessDeterministic(t *testing.T) {
	...
	run := func() ([]Artifact, []simulation.Event, error) {
		...
		copy(evs, events)
		err := PostProcess(arts, evs, SignificanceContext{}, TransferContext{}, nil)
		return arts, evs, err
	}
...
func TestPostProcessSignificanceFigureAnnualAccrual(t *testing.T) {
	...
	if err := PostProcess(artifacts, events, sig, TransferContext{}, nil); err != nil {
		t.Fatalf("PostProcess: %v", err)
	}
...
func TestPostProcessSignificanceSettlementLumpSum(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.class, func(t *testing.T) {
			...
			if err := PostProcess(artifacts, nil, sig, TransferContext{}, nil); err != nil {
				t.Fatalf("PostProcess: %v", err)
			}
...
func TestPostProcessSignificanceDeterministic(t *testing.T) {
	...
	run := func() ([]Artifact, []simulation.Event, error) {
		...
		evs := make([]simulation.Event, len(events))
		copy(evs, events)
		err := PostProcess(arts, evs, sig, TransferContext{}, nil)
		return arts, evs, err
	}
